...
{
    public class AndPlugCommonSettingContent : IPlugCommonSettingContent
    {
        public Task<RenderFragment?> GetPlugCommonSettingContent(GetSettingContext context)
        {
            ...
            if (context.PlugTypeKey == PlugKeySetting.CommonSettingPageKey)
            {
                ...
                return Task.FromResult<RenderFragment?>(builder =>
                {
                    ...
                    builder.OpenComponent<AndPlugCommonSettingPage>(sequence++);
                    //builder.AddAttribute(sequence++, nameof(AndPlugCommonSettingPage.Plug), Plug);
        ...
        public Task<PlugSettings?> GetPlugBaseSetting()
        {
            ...
            {
                Name = InitVariableNames.ConditionExpressionA.ToString(),
                Type = VariableTypeEnum.ConditionExpression.ToString(),
                IsBrowsable = true
            });
            InitVariables.Add(new BaseVariable()
            {
                Name = InitVariableNames.ConditionExpressionB.ToString(),
                Type = VariableTypeEnum.ConditionExpression.ToString(),
                IsBrowsable = true
            });
