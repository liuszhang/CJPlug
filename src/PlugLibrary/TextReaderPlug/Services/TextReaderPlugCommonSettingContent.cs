...
namespace TextReaderPlug.Services
{
    public class TextReaderPlugCommonSettingContent : IPlugCommonSettingContent
    {
        public Task<RenderFragment?> GetPlugCommonSettingContent(GetSettingContext context)
        {
            ...
            if (context.PlugTypeKey == PlugKeySetting.CommonSettingPageKey)
            {
                ...
                return Task.FromResult<RenderFragment?>(builder =>
                {
                    ...
                    builder.OpenComponent<TextReaderPlugCommonSettingPage>(sequence++);
                    builder.SetKey(context.PlugTypeKey);
                    //builder.AddAttribute(sequence++, nameof(TextReaderPlugCommonSettingPage.PlugDefinitionId), context.PlugDefinitionId);
                    builder.AddAttribute(sequence++, nameof(TextReaderPlugCommonSettingPage.SettingContext), context);
