...
using MCDataGetPlug.Pages;
using Microsoft.AspNetCore.Components;
using CJ.Plug.ApiClient.Contracts;
using CJ.Plug.PlugBaseCore.Contracts;
using CJ.Plug.PlugBaseCore.Models;

namespace MCDataGetPlug.Services
{
    public class MCDataGetPlugCommonSettingContent : IPlugCommonSettingContent
    {
        private MCDataGetPlugCommonSettingPage? _designerWrapper;
        public Task<RenderFragment?> GetPlugCommonSettingContent(GetSettingContext context)
        {
            ...
            if (context.PlugTypeKey == PlugKeySetting.CommonSettingPageKey)
            {
                ...
                return Task.FromResult<RenderFragment?>(builder =>
                {
                    ...
                    builder.OpenComponent<MCDataGetPlugCommonSettingPage>(sequence++);
                    builder.SetKey(context.PlugTypeKey);
                    //builder.AddAttribute(sequence++, nameof(PythonPlugCommonSettingPage.Flowchart), flowchart);
                    builder.AddComponentReferenceCapture(sequence++, @ref => _designerWrapper = (MCDataGetPlugCommonSettingPage)@ref);
