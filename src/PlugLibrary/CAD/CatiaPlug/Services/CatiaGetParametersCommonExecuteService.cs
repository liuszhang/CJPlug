...
{
    /// <summary>
    /// Catia 获取模型参数插头通用执行服务。
    /// 继承 <see cref="StationPlugExecuteService"/> 获得两阶段状态机（提交→图站执行完成）的通用能力。
    /// ToolName 与种子 ToolSeedDataProvider.DefaultTools 的「获取Catia模型参数」逐字一致（对齐方案 6.2 三处一致）。
    /// </summary>
    public class CatiaGetParametersCommonExecuteService : StationPlugExecuteService
    {
        public CatiaGetParametersCommonExecuteService(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }

        // ========================= 抽象实现 =========================

        protected override string ToolName => "获取Catia模型参数";
        protected override string ToolVersion => "1.0";
        protected override string[]? DataPrepareVariableNames => Enum.GetNames(typeof(CatiaGetParametersVariables));
        protected override string ResultStringVariableName => CatiaGetParametersVariables.ResultString.ToString();

        public override bool IsThisPlugTypeKey(string? PlugTypeKey) => (PlugTypeKey == PlugKeySetting.CatiaGetParameters.CommonExecuteKey);
