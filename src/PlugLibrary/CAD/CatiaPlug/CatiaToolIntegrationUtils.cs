...
namespace CatiaPlug
{
    /// <summary>
    /// Catia 模型参数处理工具类。
    /// 仿 NXPlug.ToolIntegrationUtils 实现，供 Catia 主插头读参/改参时解析与合并参数。
    /// </summary>
    public class CatiaToolIntegrationUtils
    {
        /// <summary>
        /// 从 Catia 插头获取到的参数字符串中解析出参数列表
        /// </summary>
        /// <param name="parameters">图站返回的参数串（name=value 空格分隔）</param>
        /// <param name="OldModelParameters">原有参数列表（保留输入输出配置）</param>
        /// <returns></returns>
        public static List<ModelParameter>? ProcessGetParameters(string? parameters, List<ModelParameter>? OldModelParameters = null)
        {
            ...
            foreach (var i in paras)
            {
                try
                {
                    var tmpCatiaInput = new ModelParameter { Name = i.Split('=')[0], Value = i.Split('=')[1], SettingValue = null, IsInput = false };
                    ModelParameters.Add(tmpCatiaInput);
                }
                catch (Exception ex)
                {
        ...
        /// <summary>
        /// 合并参数生成用于 CatiaSetParameters 的新参数字符串
        /// </summary>
        /// <param name="ModelParameters">参数列表</param>
        /// <param name="PlugDefinitionId">插头定义ID</param>
        /// <param name="PlugDataZone">数据空间</param>
        /// <returns>新参数字符串（name=value, 逗号分隔）；无变化返回 null</returns>
