...
{
    public class DefaultInputMappingTypeHandler : IVariableUIHandler
    {
        public bool GetSupportsUIHint(string uiHint)
        {
            return uiHint == VariableTypeEnum.DefaultInputMapping.ToString();
        }
        ...
        public RenderFragment? GetArrayChildContent(DisplayInputEditorContext context)
        {
            return builder =>
            {
                builder.OpenComponent(0, typeof(DefaultInputMappingTable));
                builder.AddAttribute(1, nameof(DefaultInputMappingTable.EditorContext), context);
                builder.CloseComponent();
            };
        }
    }
