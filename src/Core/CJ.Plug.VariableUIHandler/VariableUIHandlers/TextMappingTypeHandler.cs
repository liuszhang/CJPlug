...
{
    public class TextMappingTypeHandler : IVariableUIHandler
    {
        public bool GetSupportsUIHint(string uiHint)
        {
            return uiHint == VariableTypeEnum.TextMapping.ToString();
        }
        ...
        public RenderFragment? GetArrayChildContent(DisplayInputEditorContext context)
        {
            return builder =>
            {
                builder.OpenComponent(0, typeof(TextMappingTable));
                builder.AddAttribute(1, nameof(TextMappingTable.EditorContext), context);
                builder.CloseComponent();
            };
        }
    }
