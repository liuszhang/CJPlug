...
{
    public class WordTextMappingTypeHandler : IVariableUIHandler
    {
        public bool GetSupportsUIHint(string uiHint)
        {
            return uiHint == VariableTypeEnum.WordTextMapping.ToString();
        }
        ...
        public RenderFragment? GetArrayChildContent(DisplayInputEditorContext context)
        {
            return builder =>
            {
                builder.OpenComponent(0, typeof(WordTextMappingTable));
                builder.AddAttribute(1, nameof(WordTextMappingTable.EditorContext), context);
                builder.CloseComponent();
            };
        }
    }
