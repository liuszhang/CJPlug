...
{
    public class RequestHeaderTypeHandler : IVariableUIHandler
    {
        public bool GetSupportsUIHint(string uiHint)
        {
            return uiHint == VariableTypeEnum.RequestHeader.ToString();
        }
        ...
        public RenderFragment? GetArrayChildContent(DisplayInputEditorContext context)
        {
            return builder =>
            {
                builder.OpenComponent(0, typeof(RequestHeaderTable));
                builder.AddAttribute(1, nameof(RequestHeaderTable.EditorContext), context);
                builder.CloseComponent();
            };
        }
    }
