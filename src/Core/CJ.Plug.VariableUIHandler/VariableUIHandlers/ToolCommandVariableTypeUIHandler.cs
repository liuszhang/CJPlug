...
{
    public class ToolCommandVariableTypeUIHandler : IVariableUIHandler
    {
        public bool GetSupportsUIHint(string uiHint)
        {
            return uiHint == VariableTypeEnum.ToolCommandVariable.ToString();
        }
        ...
        public RenderFragment? GetArrayChildContent(DisplayInputEditorContext context)
        {
            return builder =>
            {
                builder.OpenComponent(0, typeof(ToolCommandVariableTable));
                builder.AddAttribute(1, nameof(ToolCommandVariableTable.EditorContext), context);
                builder.CloseComponent();
            };
        }
    }
